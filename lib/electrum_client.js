				...
				try {
					...
					if (responseLength > 0) {
						for (let i = 0; i < responseLength; i++) {
							const { height, tx_hash } = response[i];
		...
		try {
			...
			await Promise.all(scripthashes.map(async ({ scriptHash = "", address = "", path = "" } = {}) => {
				try {
					...
					if (responseLength > 0) {
						for (let i = 0; i < responseLength; i++) {
							const { height, tx_hash, tx_pos, value } = response[i];
